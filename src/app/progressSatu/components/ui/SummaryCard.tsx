...
const SummaryCard: React.FC = () => {
  ...
  useEffect(() => {
    const fetchAntrianData = async () => {
      if (nomorTelepon) {
        try {
          const response = await fetch(`/api/antrian?nomorTelepon=${nomorTelepon}`);
    ...
    <div className="flex justify-between gap-6 p-6 rounded-lg w-full max-w-6xl mx-auto bg-white">
      <div className="flex items-start gap-4">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">{antrianData?.tipeMobil || "Loading..."}</p> {/* Display tipeMobil here */}
        </div>
      </div>
      ...
      <div className="flex flex-col items-end">
        <div className="flex justify-between w-52">
          ...
          <p className="text-gray-900 text-sm">
            {antrianData && antrianData.totalIncome
